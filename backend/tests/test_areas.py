...
def test_create_area(client: TestClient, project: Project) -> None:
...
def test_create_area_invalid_project(client: TestClient) -> None:
    response = client.post(
...
def test_list_areas_empty(client: TestClient, project: Project) -> None:
    response = client.get(f"/api/v1/projects/{project.id}/areas")
    assert response.status_code == 200
    assert response.json() == []


def test_list_areas_by_project(client: TestClient, project: Project) -> None:
    client.post(
        f"/api/v1/projects/{project.id}/areas",
        json={"name": "Área A"},
    )
...
def test_list_areas_scoped_to_project(
    client: TestClient, project: Project, organization: Organization
) -> None:
...
def test_get_area(client: TestClient, project: Project) -> None:
...
def test_get_area_not_found(client: TestClient) -> None:
    response = client.get(f"/api/v1/areas/{uuid4()}")
    assert response.status_code == 404


def test_update_area(client: TestClient, project: Project) -> None:
    create_resp = client.post(
        f"/api/v1/projects/{project.id}/areas",
...
def test_update_area_not_found(client: TestClient) -> None:
    response = client.patch(
...
def test_delete_area(client: TestClient, project: Project) -> None:
    create_resp = client.post(
        f"/api/v1/projects/{project.id}/areas",
    ...
    get_response = client.get(f"/api/v1/areas/{area_id}")
    assert get_response.status_code == 404


def test_delete_area_not_found(client: TestClient) -> None:
    response = client.delete(f"/api/v1/areas/{uuid4()}")
    assert response.status_code == 404
