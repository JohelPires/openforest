...
export function Sidebar({ onNavigate }: SidebarProps) {
   ...
   const workspaceMeta = organization ? ROLE_LABEL[organization.role] : user ? 'Conta' : null
   ...
   return (
      <div className="flex h-full flex-col">
         ...
         <nav aria-label="Monitoramento" className="flex-1 overflow-y-auto px-3 py-5">
            <p className="px-2 font-mono text-[10px] font-medium uppercase tracking-[0.2em] text-gold">Monitoramento</p>
               ...
               {MONITORING_NAV.map((item) => (
                  <li key={item.href}>
                     ...
                     >
                        ...
                        <item.icon
                           className={cn('h-4 w-4 shrink-0', isActive(item.href) ? 'text-forest' : 'text-moss/70')}
                           aria-hidden="true"
                        />
                        {item.label}
                     </Link>
            ...
            <p className="mt-7 px-2 font-mono text-[10px] font-medium uppercase tracking-[0.2em] text-gold">Acervo</p>
            <ul className="mt-2 space-y-0.5">
         ...
         <div className="border-t border-forest/10 px-3 py-4">
            <div className="flex items-center gap-3 px-2">
               ...
               <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-forest">{workspaceName ?? 'Conta'}</p>
                  {workspaceMeta ? (
